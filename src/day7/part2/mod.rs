...
impl Deck {
    ...
    fn determine_deck_type(s:String) -> DeckType {
        ...
        if test_map.contains_key(&'J') && *test_map.get(&'J').unwrap() != 5{
            let key_with_max_value = test_map.iter().filter(|(key, _)| **key != 'J').max_by_key(|entry | entry.1).unwrap();
            test_map.insert(*key_with_max_value.0, key_with_max_value.1+test_map.get(&'J').unwrap());
            test_map.remove(&'J');
        }
